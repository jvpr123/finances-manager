...
import { makeFakeCreateTransactionRequest } from "src/__tests__/utils/http/transactions/HttpTransactionsMocks.factory";
...
const makeUseCaseStub = (): ICreateTransactionUseCase => ({
  execute: jest.fn().mockResolvedValue(makeFakeTransaction()),
});

const makeSUT = (useCase: ICreateTransactionUseCase): IController =>
  new CreateTransactionController(useCase);

describe("Create User Controller", () => {
  let sut: IController;
  let useCase: ICreateTransactionUseCase;

  const httpRequest = makeFakeCreateTransactionRequest();
  ...
  describe("Dependency: CreateTransactionUseCase", () => {
    ...
    it("should return a 201 status-code response when useCase operation succeeds", async () => {
      expect(sut.handle(httpRequest)).resolves.toEqual(
        created({ transaction: makeFakeTransaction() })
      );
    });
  });
});
