...
import { makeFakeUpdateTagRequest } from "src/__tests__/utils/http/tags/HttpTagsMocks.factory";
...
const makeUseCaseStub = (): IUpdateTagUseCase => ({
  execute: jest.fn().mockResolvedValue(makeFakeTag()),
});

const makeSUT = (useCase: IUpdateTagUseCase): IController =>
  new UpdateTagController(useCase);

describe("Update Tag Controller", () => {
  let sut: IController;
  let useCase: IUpdateTagUseCase;

  const httpRequest = makeFakeUpdateTagRequest();

  beforeEach(() => {
    useCase = makeUseCaseStub();
    sut = makeSUT(useCase);
  });
  describe("Dependency: UpdateTagUseCase", () => {
    it("should call execute() method from updateTagUseCase with correct values", async () => {
    ...
    it("should return a 200 status-code response when useCase operation succeeds", async () => {
      expect(sut.handle(httpRequest)).resolves.toEqual(
        ok({ updated: true, tag: makeFakeTag() })
      );
    });
  });
});
