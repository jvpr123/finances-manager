...
import { FindUnitByNameController } from "./FindUnitByName.controller";

import * as errorHandler from "src/errors/utils/Handler.error";
import { ValidationError } from "src/errors/Validation.error";
import { NotFoundError } from "src/errors/NotFound.error";

const makeUseCaseStub = (): IFindUnitByNameUseCase => ({
  execute: jest.fn().mockResolvedValue(makeFakeUnit()),
});

const makeSUT = (useCase: IFindUnitByNameUseCase): IController =>
  new FindUnitByNameController(useCase);

describe("Create Unit Controller", () => {
  let sut: IController;
  let useCase: IFindUnitByNameUseCase;

  const httpRequest = { params: { name: "unit_name" } };
  ...
  describe("Dependency: FindUnitByNameUseCase", () => {
    it("should call execute() method from findUnitByNameUseCase with correct values", async () => {
